...
class DualContouringVertexStrategy3x3(DualVertexStrategy):
    ...
    def find_vertex_locations(
        ...
        for off, p, n, bias in zip(sijk, points, normals, bias_verts):
            # Offset points to local voxel coordinates
            q = p - off[None, :]  # [0,1) range in each dim
            mask = np.isfinite(q).all(-1)  # Skip non-active voxel edges

            # Select valid intersection points and normals
            q_valid = q[mask]
            n_valid = n[mask]

            # Compute A^T A and A^T b
            AtA = np.sum([np.outer(ni, ni) for ni in n_valid], axis=0)
            Atb = np.sum([ni * np.dot(ni, qi) for ni, qi in zip(n_valid, q_valid)], axis=0)

            # Apply bias if needed
            if bias_always or (bias_failed and np.linalg.cond(AtA) > 1e12):
                if bias is not None and self.bias_strength > 0.0:
                    lambda_I = self.bias_strength * np.eye(3)
                    AtA += lambda_I
                    Atb += self.bias_strength * (bias - off)

            # Solve the normal equations
            try:
                x = np.linalg.solve(AtA, Atb)
            except np.linalg.LinAlgError:
                # Handle singular matrix (e.g., insufficient data), fallback to bias vertex
                x = (bias - off) if bias is not None else np.zeros(3)

            # Ensure the vertex is within the voxel
            x = np.clip(x, 0.0, 1.0)
            verts.append(x + off)
...
class DualContouringVertexStrategy(DualVertexStrategy):
    ...
    def find_vertex_locations(
        ...
        for off, p, n, bias in zip(sijk, points, normals, bias_verts):
            ...
            verts.append(x + off)
        print("sols ",sols, len(verts), len(points))
